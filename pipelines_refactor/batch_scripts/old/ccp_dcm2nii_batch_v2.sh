...
# This version for series grouped by phase (each  folder contains only one phase encoding of task or rest series)
...
total_rest=$(($(grep -ic "rfMRI" "${LIST}")/2))
total_task=$(($(grep -ic "tfMRI" "${LIST}")/2))
...
#        	${CCP_DCM2NII} --DicomFolder="$IN_DIR/${num_seriesSPIN[$idx]}/DICOM" --OutDir="$TDIR" > /dev/null 2>&1
		${CCP_DCM2NII} --DicomFolder="$IN_DIR/${num_seriesSPIN[$idx]}/DICOM" --OutDir="$TDIR" > "${TDIR}/${num_seriesSPIN[$idx]}.log"
...
#			echo "NII.GZ filename\t:\t${filename}"
			...
			cp "${TDIR}/${num_seriesSPIN[$idx]}.log" "${nii_folder}/${nameSPIN[$idx]}.log"
...
for ((i=1; i<=${total_t1}; ++i )) ; do
	...
	echo -e "Converting series ${IN_DIR}/${num_seriesT1W[$idx]}/DICOM to ${nii_folder}/${PREFIX}T1w_MPR${i}.nii.gz"
        ${CCP_DCM2NII} --DicomFolder="$IN_DIR/${num_seriesT1W[$idx]}/DICOM" --OutDir="${TDIR}" > "${TDIR}/${num_seriesT1W[$idx]}.log"
        nii_exists=$(extension_exists "nii" "${TDIR}")
	echo -e "\nNII exists?\t:\t${nii_exists}"
		...
		cp "${TDIR}/${num_seriesT1W[$idx]}.log" "${nii_folder}/${nameT1W[$idx]}.log"
...
for ((i=1; i<=$total_t2; ++i )) ; do
	...
	echo -e "Converting series ${IN_DIR}/${num_seriesT2W[$idx]}/DICOM to ${nii_folder}/${PREFIX}T2w_SPC${i}.nii.gz"
        ${CCP_DCM2NII} --DicomFolder="$IN_DIR/${num_seriesT2W[$idx]}/DICOM" --OutDir="$TDIR" > "${TDIR}/${num_seriesT2W[$idx]}.log"
	...
	if $nii_exists ;then #if there is a nii, gzip it (gaziop failed with the option '-g y' used in dcm2nii directly for very large files)
		...
		echo -e "NII.GZ filename\t:\t${filename}"
                ...
                cp "${TDIR}/${num_seriesT2W[$idx]}.log" "${nii_folder}/${nameT2W[$idx]}.log"
...
##########
echo ${total_rest}
for ((i=1; i<=$total_rest; ++i )) ; do              # each i --> $ 4 rest series (RL/AP standard and ref, LR/PA standard and Ref) --> not necessarily. Let's change this for pairs

	restname=$(echo "${nameREST[2*${i}-2]}" | awk -F'_' '{print $2}')
	polarity=$(series_polarity "${nameREST[2*${i}-2]}")
	mkdir -p "${OUT_DIR}/rfMRI_${restname}${polarity}"
		...
		echo -e "\n\n\n"
		echo -e "Converting series ${IN_DIR}/${num_seriesREST[$idx]}/DICOM to NIFTI; series name\t:\t$OUT_DIR/rfMRI_${restname}${polarity}"
        	${CCP_DCM2NII} --DicomFolder="$IN_DIR/${num_seriesREST[$idx]}/DICOM" --OutDir="$TDIR" > "${TDIR}/${num_seriesREST[$idx]}.log"
		nii_exists=$(extension_exists "nii" "$TDIR") 
		echo -e "\nNII exists?\t:\t${nii_exists}"
			...
			if is_sbref ${nameREST[$idx]}; then
            			newname="rfMRI_${restname}${polarity}_SBRef.nii.gz"
        		else
            			newname="rfMRI_${restname}${polarity}.nii.gz"
        		fi

                	cp "${filename}" "$OUT_DIR/rfMRI_${restname}${polarity}/${PREFIX}${newname}"
	                cp "${TDIR}/${num_seriesREST[$idx]}.log" "$OUT_DIR/rfMRI_${restname}${polarity}/${newname}.log"
                	printf "%s %s\n" "${lineREST[$idx]}" "$OUT_DIR/rfMRI_${restname}${polarity}/${PREFIX}${newname}"  >> $TFILE
...
for ((i=1; i<=${total_task}; ++i )) ; do
	...
	task=$(echo "${nameTASK[2*${i}-2]}" | awk -F'_' '{print $2}')
	polarity=$(series_polarity "${nameTASK[2*${i}-2]}")
	mkdir -p "${OUT_DIR}/tfMRI_${task}${polarity}"
		...
		echo -e "Converting series ${IN_DIR}/${num_seriesTASK[$idx]}/DICOM to NIFTI; series name\t:\t$OUT_DIR/tfMRI_${task}${polarity}"
        	${CCP_DCM2NII} --DicomFolder="$IN_DIR/${num_seriesTASK[$idx]}/DICOM" --OutDir="$TDIR" > "${TDIR}/${num_seriesTASK[$idx]}.log"
		...
		echo -e "\nNII exists?\t:\t${nii_exists}"
        	...
        	if $nii_exists; then
        		...
        		gzip $filename # compress out of dcm2nii, because dcm2nii failed due to file size
	                ...
	                cp "${TDIR}/${num_seriesTASK[$idx]}.log" "$OUT_DIR/tfMRI_${task}${polarity}/${newname}.log"
